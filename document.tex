...
a hand gesture classification algorithm to recognize control commands and an open source gaze estimation algorithm to
...
identification purposes by the system. Finally, a hand gesture is mapped to a specific control command that makes the
...
subject is moving around an environment. Eye trackers are used in research on the visual system, in psychology, in
cognitive linguistics, product design and in Human Computer Interaction (HCI) research \cite{myiwann2011}. There
are a number of methods for measuring eye movement. The most popular variant uses video images from which the eye
position is extracted. Other methods use search coils or are based on electrooculography.
...
communicate with objects without the need for external control devices. Hand gestures are an obvious choice as a mechanism
to interact with objects in the environment \cite{Rozado2012b,myicann2010}.
...
and articulated objects like hands represent added difficulty both for segmentation and shape recognition
purposes.
...
as holding the hand with a preset number of fingers for a predefined dwell time and moving it in a particular direction:
...
We used an open source gaze tracker, described in \cite{Mardanbegi2011}, to monitor user's gaze. The gaze tracker is
called Haytham and it provides real-time gaze estimation in the user’s field of view or in the computer display by analyzing
eye movement.
 

The system recognizes objects in the environment by detecting visual markers associated to them through the scene camera
also refered to here as field of view camera. When the subject carrying the glasses gazes at an object, the visual
marker placed nearby is recognized by the visual marker recognition library and this opens a temporal window for that
object to react to hand gestures coming from the user.


To interact with an object in the environment, the user of the glasses needs to first look at the binary visual marker
...
marker associated to it, make only that object to respond to the subsequent hand gesture. The off-the-shelf components
...
investigate and stretch the possibilities of the innovative HCI paradigm.
...
used by the gaze estimation algorithms to determine the PoR of the user on the screen.}
...
system in a environment where 3 ``smart'' objects could be controlled by the system: a computer, a set of leds in a
bread board and a robot. The hand gesture recognition module could recognize 5 different states of the hands as defined
by the number of fingers being held up: 1, 2, 3, 4 and 5. A gesture was defined as one of these 5 states plus one of
four spatial directions: up, down, left and right.

The infrared leds could just be turned on and off. Two fingers being held up and an upward movement would turn on the
leds. 4 fingers being hold up and a movement to the right would turn them off.
...
gestures associated to different commands could be envisioned and implemented.
...
present in the rest of the room enhanced hand recognition performance.

This manuscript associated video \footnote{\url{http://www.youtube.com/watch?v=tBOfvZboGfk}} provides a good overview of
the system at work and being used by two different users to interact with a computer, a breadboard with a set of light
emitting diodes and with a robot.
...
hand gestures using a set of gaze tracking glasses. The method is easily extensible to multiple objects in the
environment and a wide array of hand gestures.

The low-cost headmounted eye tracker used and the gaze estimation algorithms employed to not compensate for parallax
error, i.e. the inability to differentiate between the working plane and the calibration plane
\cite{mardanbegi2012parallax}. This face limited the interaction with objects at a distance and objects up close.
Nonetheless, since the field of view camera used is relatively close to the eye being tracked, see Figure \ref{}, the
parallax error was minimized. Furthermore, we notice that during the calibrations, using calibrations points situated at
different distances (from 1 to 10 meters) would find a compromise between objects objects far away and objects up close
and would generate good gaze estimation for all sort of distances. We noticed that gaze estimation accuracy was never an
issue for our system. Only over time, if the glasses would move slightly from their position during calibration, due to
sweat on the skin and the glasses sliding slightly on it, would gaze estimation degrade slightly.


We did notice problems with the skin detection when the hand was position within the field of view of the scene camera
when changing illumination conditions of the background over which the hand was visualized. More sophisticated
algorithms could help to solve this issue.


An important issue of the system was the fact that the user wearing the glasses did not have any sort of feedback in
terms of where within the field of view of the scene camera the hand was place when it was about to initiate a hand
gesture. This is due to the lack of a display on the glasses to provided visual feedback in terms of how the hand is
positioned with respect to the field of view of the scene camera. We implemented an auditory feedback signal to indicate
that the system had found the hand within the field of view of the scene camera and it was therefore ready to receive a
gesture. We found it that this helped the user but still did not provide inmediate feedback to carry out small
...
Further work should strive to carry out a quantitative analysis of the performance of the system within a large user study.
More shopisticated hand gestures can also be envisioned. Yet, care should be taken in order to not mentally overload the user.
